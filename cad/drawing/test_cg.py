...
def test_metadata_part_number_for_commercial() -> None:
    ...
    spec = load_cg_spec()
    # act
    actual = build_drawing_metadata("CG", V1_COMMERCIAL_PARAMS, spec)
    # assert
...
def test_metadata_part_number_for_defense_appends_d() -> None:
    ...
    # act
    actual = build_drawing_metadata("CG", defense_params, spec)
    # assert
...
def test_export_metadata_creates_json(tmp_path: Path) -> None:
    # arrange
    spec = load_cg_spec()
    metadata = build_drawing_metadata("CG", V1_COMMERCIAL_PARAMS, spec)
